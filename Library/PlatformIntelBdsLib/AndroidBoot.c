...
)
{
  ...
  if (EFI_ERROR(Status))
    goto FREEBUFFER;
  Status = PlatformBdsAndroidLoadImage(BlockIo, off_ramdisk, AndroidHdr->ramdisk_size, &Parsed.Ramdisk, AndroidHdr->ramdisk_addr);
  if (EFI_ERROR(Status))
    goto FREEBUFFER;
  ...
  // load tags
  Status = PlatformBdsAndroidLoadImage(BlockIo, off_tags, TagsSize, &Parsed.Tags, AndroidHdr->tags_addr);
  if (EFI_ERROR(Status))
    goto FREEBUFFER;
