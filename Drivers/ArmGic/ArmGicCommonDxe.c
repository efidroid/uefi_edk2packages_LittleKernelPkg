...
typedef struct lkapi_ihandler {
	lkapi_int_handler func;
	void *arg;
} lkapi_ihandler_t;
...
{
  ASSERT(gRegisteredInterruptHandlersLK[Source].func);
...
void lkapi_int_register_handler(unsigned int vector, lkapi_int_handler func, void *arg) {
  ...
  gRegisteredInterruptHandlersLK[vector].func = func;
  ...
  )
{
  ...
  // Initialize the array for the LK Interrupt Handlers
  gRegisteredInterruptHandlersLK = (lkapi_ihandler_t*)AllocateZeroPool (sizeof(lkapi_ihandler_t) * mGicNumInterrupts);
  if (gRegisteredInterruptHandlersLK == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
