...
)
{
  ...
  case LCD_BITS_PER_PIXEL_24:
    ...
    PixelBuffer8bit = (UINT8 *) AllocatePool((Height * Width * BytesPerPixel) * sizeof(UINT8));
    ...
    SizeIn8Bits = Width * BytesPerPixel;
    ...
    for (SourcePixelY = SourceY, DestinationPixel8bit = PixelBuffer8bit;
         ...
         SourcePixelY++, DestinationPixel8bit += Width)
    {
      ...
      // Copy the entire line Y from video ram to the temp buffer
      CopyMem( (VOID *)DestinationPixel8bit, (CONST VOID *)SourcePixel8bit, SizeIn8Bits);
    }
    ...
    for (DestinationPixelY = DestinationY, SourcePixel8bit = PixelBuffer8bit;
         ...
         DestinationPixelY++, SourcePixel8bit += Width)
    {
